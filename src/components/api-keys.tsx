...
const APIKeys = () => {
  ...
  useEffect(() => {
    async function loadAPIKeys() {
      ...
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/keys`, {
        method: "GET",
        headers: {
  ...
  async function deleteAPIKey(id: string) {
    try {
      setLoading(true);
      const accessToken = await getAccessToken();
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/keys/${id}`, {
        method: "DELETE",
        headers: {
          "X-Orbiter-Token": `${accessToken}`,
          Source: "web-app",
            ...
            <>
              ...
              <div className="flex items-center justify-between p-2 bg-gray-100 rounded">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => handleCopy(tempKey?.apiKey as string, "key")}
                >
                  {copiedField === "key" ? (
